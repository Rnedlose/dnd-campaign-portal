...
export function ProfileContent({ userId }: { userId: string }) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await response.json();
      setProfile((prev: UserProfile) => ({ ...prev, image: data.imageUrl }));
              ...
              <div>
                ...
                {isEditing && isOwnProfile ? (
                  <Input
                    value={profile.name || ''}
                    onChange={(e) =>
                      setProfile((prev: UserProfile) => ({ ...prev, name: e.target.value }))
                    }
                    disabled={isLoading}
                  />
                ) : (
              ...
              <div>
                ...
                {isEditing && isOwnProfile ? (
                  <Textarea
                    value={profile.bio || ''}
                    onChange={(e) =>
                      setProfile((prev: UserProfile) => ({ ...prev, bio: e.target.value }))
                    }
                    disabled={isLoading}
                    rows={4}
                  />
